...
public class PopupPlaylistAdapter extends RecyclerView.Adapter<PopupPlaylistAdapter.PopupPlaylistViewHolder>{
    ...
    @NotNull
    @Override
    public PopupPlaylistViewHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        // 创建item的布局实例
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_popup_playlist, parent, false);
        // 返回ViewHolder实例
        return new PopupPlaylistViewHolder(itemView);
    }



    @Override
    public void onBindViewHolder(@NonNull @NotNull PopupPlaylistViewHolder holder, int position) {
       //获取当前歌曲信息
