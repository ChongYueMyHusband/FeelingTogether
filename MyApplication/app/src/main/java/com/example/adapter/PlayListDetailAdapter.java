...
public class PlayListDetailAdapter extends RecyclerView.Adapter<PlayListDetailAdapter.PlayListDetailAdapterHolder>{
    ...
    @NotNull
    @Override
    public PlayListDetailAdapterHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        // 创建item的布局实例
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_playlist_deatil, parent, false);
        // 返回ViewHolder实例
        return new PlayListDetailAdapterHolder(itemView);
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull PlayListDetailAdapterHolder holder, @SuppressLint("RecyclerView") int position) {
