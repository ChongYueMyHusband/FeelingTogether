...
*/
    ...
    public Result<User> login(@RequestBody @Valid User user, BindingResult errors) {
        ...
        if (result.getData().getHeadPortraitBase64Code() == null) {
            String projectRoot = System.getProperty("user.dir");
            String Path = Paths.get(projectRoot, "resource/Head/default.png").toString();
            ...
            try (FileInputStream imageInputStream = new FileInputStream(imageFile)) {
                ...
                user.setHeadPortraitBase64Code(base64String);
                result.setData(user);
   ...
   @PostMapping("/updateFace")
    public Result<User> updateHeadPortrait(@RequestBody  User user) {
        ...
        Result<User> result=new Result<>();
       String projectRoot = System.getProperty("user.dir");
       String Path  = Paths.get(projectRoot, "resource/Head/face/currentImg.png").toString();
       String Base64Code= user.getHeadPortraitBase64Code();
       byte[] imageBytes = Base64.decodeBase64(Base64Code);
       File imageFile = new File(Path);
       try (OutputStream outputStream = new BufferedOutputStream(new FileOutputStream(imageFile))) {
